import { HttpClient, HttpHeaders } from '@angular/common/http';
 ...
 isAdminUser(): boolean {
  ...
  getRecipes(): Observable<any> {
    return this._httpClient.get(`${this.baseUrl}/app/recipe/?MaxResultCount=66`); // MAKE SURE THIS ENDPOINT IS CORRECT
  }
  ...
  // Tenant Endpoints (rest are commented out, assuming you're not using them yet)
  ...
  // Product Endpoints (external and internal - ensure you use the correct one in your components)
  getSingleProduct(id: number): Observable<any> {
   return this._httpClient.get(`https://dummyjson.com/products/${id}`);
  }

  // Recipe Endpoints (external and internal - ensure you use the correct one in your components)
  // (Your own API methods are getRecipes and getSingleRecipe above)

  // FitnessInfo Endpoints
  getFitnessInfo(): Observable<any> {
   return this._httpClient.get(`${this.baseUrl}/app/fitness-info`);
  }

  createFitnessInfo(fitnessData: any): Observable<any> {
   const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
   return this._httpClient.post(`${this.baseUrl}/app/fitness-info`, fitnessData, { headers });
  }
