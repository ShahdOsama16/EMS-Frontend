...
@Component({
  ...
  styleUrls: ['./zumba.component.css'], // ✅ تصحيح styleUrl -> styleUrls
})
export class ZumbaComponent implements OnInit, OnDestroy {
  backWorkouts: Workout[] = [];
  private apiUrl = 'https://passantmohamed-001-site1.mtempurl.com/api/app/workout?MaxResultCount=36';
  private subscription: Subscription | undefined;

  constructor(
    private apiService: ShareDataApiService, // متروك هنا لاحتياجات مستقبلية
    private sanitizer: DomSanitizer,
    private http: HttpClient
  ) {}

  ngOnInit(): void {
    this.fetchBackWorkouts();
  }
  ...
  fetchBackWorkouts(): void {
    this.subscription = this.http.get<{ items: Workout[] }>(this.apiUrl).subscribe(
      (response) => {
        this.backWorkouts = response.items.filter(workout => workout.category?.toLowerCase() === 'back');
        console.log('Fetched Back Workouts:', this.backWorkouts);
      },
      (error) => {
        console.error('Error fetching Back workouts:', error);
      }
    );
  }
  ...
  getSafeVideoUrl(url: string | null | undefined): SafeResourceUrl | null {
    ...
    // ✅ دعم YouTube Shorts وwatch وVimeo
